...
class Steam:
    ...
    async def _app_info(self, gid):
        ...
        if data[str(gid)]['success']:
            ...
            info['header_image'] = data ['header_image']
            info['appid'] = str(gid)
            
            platforms = data['platforms']
            info['platforms'] = ""
            
            if bool(platforms['linux']):
            	info['platforms'] += "Linux"
            	
            if bool(platforms['mac']):
            	if not bool(platforms['linux']):
            		info['platforms'] += "Mac"
            	else:
            		info['platforms'] += ", Mac"
            
            print (bool(platforms['windows']))
            if bool(platforms['windows']):
            	if not bool(platforms['linux']) and not bool(platforms['mac']):
    ...
    async def _steam(self, context, *game: str):
        """Gets info about a particular steam game."""
        game = " ".join(game)
        ...
        if match:
            ...
            if info:
                ...
                em.set_thumbnail(url=info['header_image'])
                ...
                em.add_field(name="**Platforms**", value=info['platforms'])
    ...
    async def _update(self, context):
        """Updates the list of steam games."""
        try:
